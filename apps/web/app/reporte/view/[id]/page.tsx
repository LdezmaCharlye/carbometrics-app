...
import { Leaf, ShieldCheck, Download } from "lucide-react";
...
export default function PublicReportPage() {
  ...
  const removals: any[]   = data.removals ?? [];
  ...
  const publicUrl = `https://carbometrics.site/reporte/view/${id}`;

  const baseYear = company?.baseYear ?? years[0] ?? null;
  const activeSources   = (company?.emissionSources ?? []).filter((s: any) => !s.isExcluded);
  const excludedSources = (company?.emissionSources ?? []).filter((s: any) => s.isExcluded);

  const qualityCounts = logs.reduce((acc: any, l: any) => {
  ...
  const unverifiedLogs = logs.filter((l: any) => !l.isVerified);

  const totalRemovals = removals.reduce((acc: number, r: any) => acc + Number(r.tCO2eRemovedPerYear), 0);
  ...
  return (
    <>
      ...
      <div className="no-print bg-white border-b border-gray-200 px-6 sticky top-0 z-10">
        ...
        <main id="report-content" className="max-w-3xl mx-auto px-4 py-6 space-y-4">
            ...
            <div className="relative px-8 py-8 text-center">
              ...
              <div className="inline-block bg-gray-50 rounded-2xl px-8 py-4 border border-gray-200">
                ...
                <p className="text-xs font-semibold text-green-600 mt-2">
                  {branchName ? `Instalación: ${branchName}` : "Todas las instalaciones"}
                </p>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-sm font-bold text-gray-900 border-b border-gray-100 pb-3 mb-4 uppercase tracking-wider">
              {summaries.length + 6}. Remociones y Sumideros de GEI
            </h2>
            <p className="text-sm text-gray-600 mb-4">
              Conforme a la cláusula 5.3 de la norma ISO 14064-1:2018, se declaran a continuación las remociones de GEI identificadas por <strong>{company?.name}</strong> durante el período del inventario.
            </p>
            {removals.length === 0 ? (
          ...
          {(() => {
            ...
            const s1Total = summaries.reduce((a: number, s: any) => a + getScopeTotal(s.byMonth, "SCOPE_1"), 0);
            const s2Total = summaries.reduce((a: number, s: any) => a + getScopeTotal(s.byMonth, "SCOPE_2"), 0);
            const s3Total = summaries.reduce((a: number, s: any) => a + getScopeTotal(s.byMonth, "SCOPE_3"), 0);
            const totalAll = s1Total + s2Total + s3Total;
            const hasHFCs = activeSources.some((s: any) => s.name.toLowerCase().includes("hfc") || s.name.toLowerCase().includes("refriger"));
            const hasPFCs = activeSources.some((s: any) => s.name.toLowerCase().includes("pfc") || s.name.toLowerCase().includes("fluorad"));
            const hasSF6  = activeSources.some((s: any) => s.name.toLowerCase().includes("sf6") || s.name.toLowerCase().includes("hexaflu"));
            ...
            if (s1Total > 0 && (s1Total / Math.max(totalAll, 1)) > 0.3) {
              medidas.push({ titulo: "Sustitución de combustibles fósiles", descripcion: "Las emisiones directas (Alcance 1) representan una parte significativa del inventario. Se recomienda evaluar la transición a vehículos eléctricos o híbridos, el uso de biocombustibles certificados y la optimización de rutas de transporte para reducir el consumo de combustible." });
              medidas.push({ titulo: "Eficiencia en combustión estacionaria", descripcion: "Implementar mantenimiento preventivo en generadores y equipos de combustión para optimizar la eficiencia térmica. Considerar la sustitución de generadores diésel por sistemas de energía renovable (solar, eólica) donde sea técnicamente viable." });
            }
            if (s2Total > 0 && (s2Total / Math.max(totalAll, 1)) > 0.1) {
              medidas.push({ titulo: "Eficiencia energética y energías renovables", descripcion: "Las emisiones indirectas por electricidad (Alcance 2) pueden reducirse mediante auditorías energéticas, reemplazo de iluminación por tecnología LED, instalación de sistemas de generación solar fotovoltaica y contratación de energía eléctrica de fuentes renovables certificadas." });
            }
            if (s3Total > 0) {
              medidas.push({ titulo: "Gestión de viajes y cadena de valor", descripcion: "Reducir emisiones de Alcance 3 mediante políticas de viajes corporativos (preferencia por videoconferencias), selección de proveedores con compromisos climáticos verificables y optimización logística para reducir el transporte de mercancías." });
            }
            if (hasHFCs || hasPFCs || hasSF6) {
              medidas.push({ titulo: "Control de emisiones fugitivas de gases fluorados", descripcion: "Los gases refrigerantes (HFCs, PFCs) y el hexafluoruro de azufre (SF₆) tienen potenciales de calentamiento global extremadamente altos. Se recomienda implementar programas de mantenimiento preventivo de equipos de refrigeración y aire acondicionado, registro de recargas y sustitución progresiva por refrigerantes de bajo GWP." });
            }
            if (estimatedPct > 50) {
              medidas.push({ titulo: "Mejora en la calidad del dato de actividad", descripcion: `El ${estimatedPct}% de los registros están basados en estimaciones. Se recomienda implementar sistemas de medición directa (medidores de consumo, odómetros, contadores de energía) y exigir facturas digitales como respaldo documental para mejorar la precisión del inventario.` });
            }
            if (verifiedPct < 50) {
              medidas.push({ titulo: "Fortalecimiento de la trazabilidad y verificación", descripcion: `Solo el ${verifiedPct}% de los registros han sido verificados internamente. Se recomienda establecer un proceso formal de verificación periódica de los datos de actividad, designar un responsable del inventario GEI y considerar la verificación externa por un organismo acreditado conforme a ISO 14064-3.` });
            }
            medidas.push({ titulo: "Establecimiento de metas de reducción", descripcion: "Conforme a la ISO 14064-1 y el GHG Protocol, se recomienda establecer metas cuantificadas de reducción de emisiones alineadas con los Acuerdos de París (reducción del 45% al 2030 respecto al año base), con seguimiento anual mediante este inventario." });
